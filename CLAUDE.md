...
- **Budget caps** — `budget.daily_tokens` enforces a per-workspace per-UTC-day token ceiling (summed from `usage` task events across triage + coding + evaluator) checked before triage; fails open on metering error. The paper's "cap before you ship". Usage is summed across all turns of each run (harness ≥ the run-total-usage change emits accumulated input+output+cache on the terminal `EventDone`); the only gap is providers that don't report usage at all, whose runs aren't counted.
